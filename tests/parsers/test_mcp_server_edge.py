...
class TestMalformedInput:
    ...
    def test_empty_python_file_no_match(self, parser: McpServerParser, tmp_path: Path) -> None:
        """An empty .py file should not match as an MCP server."""
        (tmp_path / "server.py").write_text("")
        assert parser.can_parse(tmp_path) is False

    def test_binary_file_no_crash(self, parser: McpServerParser, tmp_path: Path) -> None:
    ...
    def test_malformed_package_json(self, parser: McpServerParser, tmp_path: Path) -> None:
    ...
    def test_parse_nonexistent_path(self, parser: McpServerParser, tmp_path: Path) -> None:
        """Parsing a non-existent directory returns empty list."""
        fake_path = tmp_path / "does_not_exist"
        result = parser.parse(fake_path)
        assert result == []

    def test_parse_file_instead_of_dir(self, parser: McpServerParser, tmp_path: Path) -> None:
...
class TestEmptyAndMinimal:
    ...
    def test_mcp_import_only_no_tools(self, parser: McpServerParser, tmp_path: Path) -> None:
    ...
    def test_empty_dir_parse_returns_empty(self, parser: McpServerParser, tmp_path: Path) -> None:
        """Parsing an empty directory returns no skills."""
        assert parser.parse(tmp_path) == []

    def test_ts_import_only_no_tools(self, parser: McpServerParser, tmp_path: Path) -> None:
...
class TestMixedFormats:
    ...
    def test_parses_both_py_and_ts(self, parser: McpServerParser, tmp_path: Path) -> None:
    ...
    def test_non_mcp_python_files_ignored(self, parser: McpServerParser, tmp_path: Path) -> None:
        """Non-MCP Python files in the same directory are skipped."""
        mcp_source = "from mcp.server import Server\napp = Server('real')\n"
...
class TestAlternateImportStyles:
    ...
    def test_from_mcp_import_server(self, parser: McpServerParser, tmp_path: Path) -> None:
    ...
    def test_require_style_js(self, parser: McpServerParser, tmp_path: Path) -> None:
        """CommonJS ``require("@modelcontextprotocol/sdk")`` is detected."""
        source = 'const { Server } = require("@modelcontextprotocol/sdk/server");\n'
        (tmp_path / "index.js").write_text(source)
        assert parser.can_parse(tmp_path) is True

    def test_js_extension_parsed(self, parser: McpServerParser, tmp_path: Path) -> None:
...
class TestCapabilityInference:
    ...
    def test_open_call_adds_filesystem(self, parser: McpServerParser, tmp_path: Path) -> None:
    ...
    def test_subprocess_adds_system_execute(self, parser: McpServerParser, tmp_path: Path) -> None:
    ...
    def test_sensitive_env_flags_credentials(self, parser: McpServerParser, tmp_path: Path) -> None:
    ...
    def test_version_defaults_to_unknown(self, parser: McpServerParser, tmp_path: Path) -> None:
    ...
    def test_name_is_file_stem(self, parser: McpServerParser, tmp_path: Path) -> None:
